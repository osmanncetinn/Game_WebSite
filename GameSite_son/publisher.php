...
$DesignerName = $_GET["DesignerName"]; // $CategoryName is array
...
if($example=="2"){
  ...
  $limit = count($DesignerName);
  foreach ($DesignerName as $row=>$value){
    $sql = "INSERT into  tbDesigner(name) values ('$value')" ;
    $result = $conn->query($sql);
  }
  // tbGameDesigner tablosuna post
  //--------------------------------------------------------------------------

  $sql = "SELECT designer_id FROM tbDesigner ORDER BY designer_id DESC LIMIT $limit"; // designer adı ile id sini aldım
  $DesignerId = $conn->query($sql); //Array olarak alıyor. Sorunsuz.

  foreach ($DesignerId as $row=>$value){
    $intvalue = (int)$value["designer_id"]; // String değerleri int yaptık.
    echo "<br>" . "designer_id" . $intvalue; // designer_id leri ekrana basıyor. Sorunsuz.
    $sql1 = "INSERT into tbGameDesigner(game_id,designer_id) values ('$GameId','$intvalue') ";
    $conn->query($sql1);
  }
  //--------------------------------------------------------------------------

  $sql = "SELECT GameDesigner_id,game_id,designer_id FROM tbGameDesigner";
  $result = $conn->query($sql);

  if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
      echo "<br>". "GameDesigner_id: " . $row["GameDesigner_id"]. " - game_id: " . $row["game_id"].  "<br>"" - designer_id: " . $row["designer_id"].  "<br>";
    }
  } else {
    echo "0 results";
  }
...
<html lang="en">
	...
	<body class="slider-collapse">
		...
		<div id="site-content">
			<div class="site-header">
              ...
              <li class="menu-item home current-menu-item"><a href="publisher.php">Publisher Ekle</a></li>
            </ul> <!-- .menu -->
			...
			<main class="main-content">
              ...
              <<form action="complete.php" target="" method="GET">
                  <table id="publisher" border=0>
                      ...
                      <tr>
                          <td>1. Publisher Bilgileri :</td>
                          <td>
                              <input type="text" name="PublisherName[]" value="" placeholder="Publisher Adı">
                          </td>
                      </tr>
                  </table>

                  <input href="add.php" type="submit" value="Submit">
                  <input type="button" value="Add Row" onclick="addRow('publisher')" />
	                <input type="button" value="Delete Row" onclick="deleteRow('publisher')" />
  ...
  function addRow(tableID) {
          ...
          var table = document.getElementById("publisher");
          ...
          element1.type = "text";
          element1.name="PublisherName[]";
          element1.placeholder = "Publisher Adı"
